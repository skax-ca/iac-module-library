...
## Priority Context
SSOT=이 repo. 엔진=OpenTofu(decisions.md 재제안 전 필독). 규약=conventions.md(이름포맷 AWS=Name태그/Azure=name인자, 노드풀명은 conventions §2-6 소유·약어카탈로그 아님). 네이밍=docs/naming/abbreviations/{aws,azure}.md(Azure 11개·5카테고리). 모듈경로=modules/<provider>/<name>/. .tf규칙=.claude/rules/terraform.md. AWS4+Azure2(vnet 릴리스·aks-cluster 설계실행완료), 전5모듈 nullable=false+태그컷 완료(2026-08-27). aks-cluster: Step1-5 실행 완료(docs 4개, main 커밋 대기), .tf 구현 미착수(별도 브랜치). 영구사실=project-memory.json.
