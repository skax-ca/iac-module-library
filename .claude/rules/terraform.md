...
⚠️ **이 파일은 이 저장소의 `.tf`에만 실린다.** `paths` 규칙은 작업 디렉토리 기준으로 매칭되므로
`--add-dir`로 붙인 배포 루트의 `.tf`를 열어도 로드되지 않는다. 아래 절 중 상당수는 배포 루트에서
뜻이 달라지거나 뒤집힌다(pre-push가 `tofu test`라는 것, 릴리스 태그와 semver, *"동작한다"* 의 기준).
배포 루트에서 지켜야 하는 것은 `CLAUDE.md` 「배포 루트 공통」의 `.tf` 작성 행이 소유한다.
