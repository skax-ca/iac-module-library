...
1. ~~**도달성 결정**~~ 🧭 **방향 확정 = bastion**(2026-08-04, §3.4 상자). **실행 대기** — `40` 개정.
...
2. **프로파일 B의 배포 자동화 수준** — helm CLI를 사람이 치는가, CI가 치는가. 후자면
   *"승인한 계획 ≠ 적용된 계획"* 구멍을 어떻게 막는지가 [`50`](50-reference-consumer-repo.md)의
   ...
   - 후보 (a) **bastion을 self-hosted runner로 겸용** — CI가 private endpoint에 닿는다.
     대가는 상시 기동 비용 + 공격 표면. (b) **수동 helm을 수용하고 한계를 명시** — 작은 고객사엔 현실적.
   - ⛔ **`40` 개정과 분리해서 결정하지 않는다**(§3.4 상자 마지막 줄). 역할 범위가 인스턴스 타입·SG·IAM을 정한다.
