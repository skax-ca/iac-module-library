...
variable "identity_id" {
  description = <<-EOT
    user-assigned managed identity의 리소스 ID. aks-cluster와 같은 경계 원칙이다.
...
variable "admin_username" {
  description = <<-EOT
    ...
    별개의 자기 신원으로 들어온다. 이 값은 브레이크글래스 계정 이름표라
...
variable "aks_entra_rbac_enabled" {
  ...
  validation {
    ...
    error_message = "aks_entra_rbac_enabled = true면 aks_cluster_name·aks_resource_group_name·identity_client_id·kubelogin_version을 전부 지정해야 한다. kubelogin convert-kubeconfig -l msi가 앞의 셋을 요구하고, 그 명령 자체를 실행할 kubelogin 바이너리 설치에 버전 핀이 필요하다. 이 값이 없으면 kubelogin이 설치되지 않아 변환이 조용히 실패하고 stale kubeconfig가 그대로 쓰인다."
  }
}
