...
이 프로젝트는 `iac-reference-infra`와 **동일한** OMC notepad 3단 구조(**Priority** 포인터·500자
이내 / **Working** 세션 서술·7일 자동 소멸 / **MANUAL** 영구 아카이브·자동 로드 안 됨)와
`project-memory.json`(구조화 영구 사실)을 쓴다(2026-08-19 전환 — 이전에는 이 repo만의 날짜별
prepend 방식을 썼으나, iac-reference-infra와 다른 메커니즘을 유지할 이유가 없어 통일했다).
...
(의도된 worktree 격리 — 버그 아님). 그래서 **iac-reference-infra를 프로젝트 루트로 연 세션
안에서는 이 repo를 대상으로 이 툴을 쓸 수 없다** — 다른 프로젝트를 세션 중간에 대상으로
지정해도 조용히 iac-reference-infra에 쓴다(2026-08-14/19 두 차례 실제 오사고 발생, 둘 다
`git checkout`으로 복구). **이 repo 작업은 iac-module-library를 프로젝트 루트로 하는 별도
Claude Code 세션에서 한다** — 그 세션 안에서는 이 절 전체가 iac-reference-infra와 완전히
동일하게 동작한다.
...
2. `remember`가 모르는, 이 repo만의 제약을 그 판단에 추가로 적용한다:
   ...
   - `docs/*.md`에는 날짜·사건 서술을 쓰지 않는다(`docs/06-conventions.md` §8 — 이 repo의
...
(iac-reference-infra의 것과 로직이 동일 — 구조가 같아졌으므로 그대로 이식했다). 위 MCP
