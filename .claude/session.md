...
**EKS를 재구축해 부모 Application 구조(#31)를 실측하고 다시 철거했다.** seed에서 addon이 하나도 안 깔린 결함 2건을 고쳤다: root-app `include`의 `applicationsets/**/*.yaml`이 바로 아래 파일을 못 잡음(eks-gitops #33), 라벨을 sprig `get`으로 읽어 `map[string]string` 타입 오류(#34, module `8868c82`). 둘 다 CI가 ArgoCD 렌더를 흉내 내지 않아 통과했다. seed 뒤 kyverno pre-delete finalizer를 부모 selfHeal이 5분마다 지우는 루프(#35, `ordering.md` `aec86a1`)와 콘솔 식별성(식별 라벨 + `resource.customLabels`, #36·#37, `gitops.md` `0e671b8`)을 고쳤다. AKS에 같은 수정 4건(aks-gitops #9~#12). workbench CLI를 v3.5.3으로(eks-ref #77·aks-ref #79).

**실측**: 설치 wave 대기(hub·dev), 해제 wave 역순(dev #39·hub #41, 각각 약 1분, ALB SG·ALB·Karpenter 잔존 0, kyverno PreDelete 훅 실행), dev Karpenter 노드 첫 기동(`r8gd.medium` spot, AMI 핀 일치, consolidation으로 회수). hub CRD health 고착(2분 반, hard refresh로 해제)은 dev에서 재현되지 않아 원인 미확정. hub 철거를 라벨 해제로 바꿨다(eks-ref `2c7d88f`), ⏳ 4곳 제거(module `ee45e9d`·eks-ref `d1fe675`). 비밀번호 교체는 생략(초기 비밀번호를 send-command로 읽음). 장표 Artifact: 「addon 라벨 전달 경로」 https://claude.ai/artifact/QbSYBYpEqizoF1Fi1k82nq
