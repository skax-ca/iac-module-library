...
| repo | git | 상태 |
|------|-----|------|
| eks-reference-infra | main = origin | **hub·dev 둘 다 철거 완료**(재구축 → 재철거. 구축은 tgw 7 → hub networking 69 → hub eks 87 → dev networking 69 → dev eks 87 → hub networking 재적용 6, 철거는 dev eks 87 → networking 69 → hub eks 87 → networking 74 → tgw 7. Flow Logs 로그 그룹 재생성분 수동 삭제 후 양 계정 `teardown-verify.sh` "잔존물 없음"). bootstrap(state 버킷·OIDC·Role)은 남아 있다. hub·spoke 모두 addon 해제는 cluster Secret `environment` 라벨 제거(`hub-lifecycle.md` 11절·`spoke-lifecycle.md` 10절, 실측 완료). workbench `argocd_version = v3.5.3`(차트 appVersion). ⚠️ 네 루트(`live/{hub,dev}/{eks,networking}`)의 `deletion_protection = false`가 `main`에 남아 있다 — 재구축이 끝나면 `true`로 되돌린다. **public**, MIT. 시크릿 0개. 최상위 `README.md` 있음(라우터: 배포 루트 5개·state key·실행 모델·문서 라우팅표). `verify.yml`(시크릿·문서주석셸·OpenTofu 3 job)이 PR·push에서 돌고 ruleset `main`이 그것을 요구한다. environment `hub`·`dev`에 required reviewer `silverte`(self-review 허용)와 브랜치 정책 `main`. plan artifact 7일. 철거 상태에서 main push는 `hub/tgw`만 plan이 성공한다(생성 7건이라 **Reject**). 나머지 4개는 `RAM Resource Share`·`TGW`·`VPC` not found로 실패한다. ⚠️ plan artifact 이름은 `eks` 루트만 `tfplan-eks-<run_id>`이고 나머지 4개는 `tfplan-<run_id>`다. 시스템 노드그룹 taint `CriticalAddonsOnly=true:NoSchedule`, 라벨 `workload-class=system`. 변수 34개 `nullable = false`. 루트마다 `backend.hcl.example`(로컬 전용 4키). 루트 5개 전부 apply job에 `apply 후 수렴 검증(읽기 전용 재-plan)`이 있다 — `hub/eks`·`dev/eks`·`dev/networking`은 ⏳ 아직 한 번도 돌지 않았다(다음 재구축이 첫 실행). **Dependabot PR 0건** — 모듈 4종이 `vpc-v0.5.0`·`eks-cluster-v0.11.0`·`workbench-v0.9.0`·`cross-account-trust-role-v0.4.0`, aws provider가 루트 5개 전부 `6.64.0`으로 수렴. 액션 6종이 커밋 SHA 핀(태그는 뒤 주석) |
| eks-platform-gitops | main = origin | **EKS 철거 상태.** **구조가 바뀌었다(#31)**: ApplicationSet은 `applicationsets/platform.yaml` 하나이고 클러스터마다 부모 `<cluster>-platform`을 만든다. 부모가 로컬 차트 `addons/platform/`으로 addon Application 10개를 wave 0(CRD) · 1(컨트롤러) · 2(CR·정책)로 렌더한다. 버전은 `addons/platform/values.yaml`의 `versions` 표(staged는 prd·nonprd 두 줄). `bootstrap/argocd-values.yaml`에 Application health Lua가 있다 — 없으면 부모가 wave를 기다리지 않는다. `decommission` 라벨은 없어졌다. **새 구조는 실측 완료**(hub seed·dev 등록·양쪽 해제). 부모에 `ignoreDifferences /metadata/finalizers` + `RespectIgnoreDifferences`(kyverno pre-delete finalizer 루프 방지), addon Application에 식별 라벨 `platform.addon`·`.cluster`·`.wave`(헬퍼 `platform.meta`)와 `resource.customLabels`. root-app include는 `applicationsets/*.yaml`, 라벨은 `index`로 읽는다(`sprig get` 금지). `clusters/dev/`는 삭제했고 `clusters/hub/`는 `environment: hub`까지 복원된 seed 입력으로 남겼다. dev 재등록은 `e71adff`(#27) 형태(`tier: nonprd`, `addon-cluster-autoscaler`·`addon-keda`, 고정 이름 `karpenterNodeRole`)를 따르고 `server`·`caData`만 새로 넣는다. ⚠️ root-app은 `prune: false`라 파일을 지워도 hub의 라이브 cluster Secret은 남는다. **public**, MIT. seed는 1 helm install → 2 AppProject → 3 cluster Secret → 4 root Application(repository Secret 단계 없음, preflight가 `repoURL`을 익명 `ls-remote`). `verify.yml`(시크릿·주석셸·매니페스트 3 job: YAML 파싱·root App `include` 판정·로컬 차트 2개 lint/template·`kyverno test`)과 ruleset. `include` 판정은 `scripts/include-check`(Go 별도 모듈, ArgoCD와 같은 `gobwas/glob v0.2.3`, 로컬 `go run -C scripts/include-check .`) — 구분자 없이 매치하므로 `*`도 `/`를 넘는다. aks-gitops도 같다. `tests/kyverno/require-karpenter-resources/` 픽스처 5건(pass 1·fail 3·skip 1). `bootstrap/argocd-seed.sh`의 SSOT가 이 저장소다. root App 렌더 리소스는 AppProject 1·Application 2·ApplicationSet 1·Secret 1(cluster Secret이 늘면 Secret도 는다). `verify.yml`이 부모 차트를 두 티어 × opt-in 유무로 렌더하고 `tier=prod`가 실패하는지 본다. Karpenter AMI 핀 `amiAliasByTier`는 prd·nonprd 둘 다 `al2023@v20260917`(nonprd `latest` 추적은 #26에서 없앴다. 새 AMI는 nonprd 줄에 먼저 올린다). Kyverno는 `policies.kyverno.io/v1beta1 ValidatingPolicy`. 훅 정규식에 `tests/`·`.github/workflows/` 포함. ⛔ 액션 SHA 핀을 걸지 않았다(OIDC job 0) |
| aks-reference-infra | main = origin | **hub·dev 둘 다 철거 완료**(dev workbench 15 → aks 6 → networking 24, hub workbench 14 → aks 8 → vwan 5 → networking 27 순 destroy, 양 구독 `teardown-verify.sh` "잔존물 없음"). state Storage Account(bootstrap)만 남아 있다. hub 구독 `57bb4b4a…`, dev 구독 `af8171fb…`(로컬 `az` 기본 구독은 dev). ⚠️ vWAN `prevent_destroy = false`와 VNet·AKS `deletion_protection = false`가 `main`에 남아 있다 — 재구축이 끝나면 `true`로 되돌린다. 재구축이 이어받는 값: k8s 1.35 private, `networkPluginMode=overlay`·`podCidr 10.244.0.0/16`·`outboundType=userAssignedNATGateway`·`networkPolicy=cilium`(dev는 hub 값 승계). provider 7루트 전부 `azurerm 5.5.0`, **Dependabot PR 0건**. workbench `argocd_version = v3.5.3`. 모듈 태그 `aks-cluster-v0.10.0`·`aks-workbench-v0.7.0`·`vnet-v0.2.0`. **public**, MIT. dev workbench `vm_size = Standard_B2s_v2` 오버라이드, SSH 키는 hub `workbench_ed25519`·dev `workbench_dev_ed25519`. ⚠️ workbench 최초 부팅의 apt lock 결함은 재구축 때 다시 나온다(`runbooks.md`에 수동 복구 절차). 철거 상태에서 main push는 루트 7개가 대상 없음으로 실패한다 |
| aks-platform-gitops | main = origin | **AKS 철거 상태.** **구조가 바뀌었다(#8)**: EKS와 같은 부모 Application 구조. wave는 0(`karpenter-nodepool`·`gateway`) · 1(`kyverno`) · 2(정책)이라 역순 해제에서 Kyverno가 NAP 노드가 살아 있을 때 지워진다. health Lua 있음. EKS에서 찾은 결함 수정 4건(include glob #9·`index` #10·finalizer 무시 #11·식별 라벨 #12)을 같이 반영했다. ⏳ AKS 실측 전. `clusters/dev/`는 삭제했고 `clusters/hub/`는 재구축용으로 남겼다. **public**, MIT. ⚠️ 재구축하면 hub ArgoCD 초기 admin 비밀번호를 다시 교체해야 하고(이번에 교체한 값은 클러스터와 함께 사라졌다), `bootstrap/argocd-values.yaml`과 cluster Secret의 client ID(`ec70a09c…`)는 새 UAMI 값으로 갱신한다. root-app은 `prune: false`라 파일을 지워도 라이브 cluster Secret은 남는다. ApplicationSet 1개(`platform`), 부모 차트 템플릿 5개. Kyverno 엔진 3.9.1(업스트림 PSS 11개 Audit + 커스텀 1개 Deny). `require-nodepool-resources`의 `namespaceSelector`는 2조건(`control-plane` 없음 + 이름이 `argocd` 아님, `f798dad`) |
...
**「재구축과 무관」 두 항목을 닫았다.** ① eks-ref의 apply 후 수렴 검증: `d99be78`이 워크플로 5개에 "수렴 검증도 있다"는 주석을 복사했는데 스텝은 hub network·tgw뿐이었다. 주석이 아니라 스텝을 맞췄다(eks-ref #78 `b20dddf`, `hub/eks`·`dev/eks`·`dev/networking`). AKS는 7루트 전부 이미 있었다. ② root App `include` CI 판정: README의 ⛔「include 판정은 흉내 내지 않는다」를 재검토했다. 기각 근거 중 "seed 뒤 `argocd app diff`"는 철거 상태에서 성립하지 않았고, "흉내는 어긋난다"는 재구현 대신 ArgoCD와 같은 `gobwas/glob v0.2.3`을 같은 방식(구분자 없음)으로 부르는 방법으로 풀었다(eks-gitops #43 `5afa634`, aks-gitops #14 `d10a35a`, module `CLAUDE.md` `1fddfac`). 대안별 0건 매치와 include 밖 매니페스트를 판정하고, 결함을 심은 사본 2건에서 실패를 확인했다. 실측 발견: gobwas는 `clusters/**/cluster-secret.yaml`이 `clusters/cluster-secret.yaml`을 잡는데 `applicationsets/**/*.yaml`은 `applicationsets/platform.yaml`을 못 잡는다(README에 반영). goTemplate 평가(#34 유형)는 범위 밖으로 남겼다.
...
- [ ] [eks-ref] **다음에 EKS를 재구축할 때**: `hub/eks`·`dev/eks`·`dev/networking`의 apply 후 수렴 검증이 처음 돈다. 종료 코드 `2`로 실패하면 스텝 결함이 아니라 그 루트의 영구 diff(예: `ignore_tags` 누락)부터 의심한다. apply 자체는 끝난 뒤라 실패해도 인프라는 섰다
