...
> ### 🔴 **문서 단위로 판정되지 않는 문서가 둘 있다 — `21`·`30`**
>
> ⇒ **이 둘을 인용할 때는 절 번호까지 쓴다.** *"21에 따르면"* · *"30에 따르면"* 은 근거가 못 된다.
>
> ⛔ **어느 절이 확정인지는 여기에 적지 않는다.** [`../README.md`](../README.md) 상태표와
> **각 문서 상단의 상태 상자**가 소유한다. `30`은 2026-08-10 2차 부분 개정으로 확정 절이 늘었고,
> **부분 개정은 앞으로도 반복된다** — 절 목록을 여기 두면 그때마다 세 곳을 고쳐야 하고,
> 위 상자가 기록한 대로 **그 구조가 두 번 stale을 만들었다.**
> 📌 같은 판단의 세 번째 적용이다: **중복 기록이 stale해지면 "더 조심하자"가 아니라 한쪽을 지운다.**
>
> 🔑 **2026-08-07 — 이것을 "예외"라고 부르지 않기로 했다.** 처음엔 `21` 하나였고 예외로 적었는데,
> `30`을 **부분 개정**하자마자 둘이 됐다. ⇒ 절 단위 판정은 예외가 아니라
> **부분 개정이라는 작업 방식의 자연스러운 결과**다.
> 📌 **그래서 규칙을 일반화한다: 설계 문서를 인용할 때는 항상 절 번호를 쓴다.**
> 문서 전체가 ✅라도 그 편이 다음 개정에 안전하다.
...
- ⛔ **공통부를 두 벌 두지 않는다.** 실제로 갈리는 지점은 **6개**이고(`21 §1.7`),
  ...
  ⚠️ **갈림점 목록은 완결이 아니다** — 초판은 5개였고, `30 §1.1` 재판정에서 6번째(저장소 접근)가 나왔다.
- ⭐ **관리형 전제 위에서 내려진 결정을 재판정할 때 물을 질문**: *"이건 우리가 고른 것인가,
  그 서비스가 준 것인가."* 후자라면 다른 경로에 **선택지 자체가 없을 수 있다.**
  실측(`30 §1.1`): CodeConnections는 우리가 고른 것처럼 서술돼 있었으나 **관리형의
  direct integration 기능**이었고, self-managed ArgoCD는 **지원조차 하지 않는다**(문서 0건).
