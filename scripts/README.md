...
| `validate-tf-comments.py` | `modules/**` 의 `.tf`·`.tftest.hcl`과 훅 2개(`.githooks/pre-commit`·`pre-push`)의 주석·`description` 산문에서 좌표 3종(절 번호 인용 기호·결정 식별자·날짜) 검사 | `.githooks/pre-commit`(해당 파일 staged 시)와 CI가 실행 |
