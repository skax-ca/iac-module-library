...
여기 있는 안은 **한 번 검토해서 값을 매겨 기각했거나, 실측이 반증한** 것이다.
...
| 하지 말 것 | 이유 |
|---|---|
| Terraform과 OpenTofu **동시 지원** | 실측 비용 5건으로 기각. 교차변수 validation 확인 비용 · lock 커밋 포기 · 로컬↔CI 피드백 지연, 그리고 `required_version`이 **두 엔진 중 느린 쪽에 묶인다** |
...
> OpenTofu 채택 근거는 **리워크 0과 조달 마찰 제거**다. 라이선스는 근거가 아니다. HashiCorp FAQ는
> 컨설턴트가 고객 프로덕션에서 BSL 제품 사용을 돕는 것을 허용한다. 고객사 비용 장벽은 **HCP/TFE
> 구독**이었고, GitHub Actions + S3로 이미 해소됐다.
...
| `lifecycle { ignore_changes }` 로 drift 눈감기 | drift를 고치지 않고 남긴다 |
| `mock_resource`로 `null`을 강제해 테스트 통과 | assertion이 모듈 대신 **mock을 검증**한다 |
...
| 모듈이 **신원과 권한 부여 리소스**(Azure user-assigned identity · role assignment) 생성 | 재사용 모듈이 만드는 리소스는 소비자 CI 신원이 그것을 만들 권한을 갖는다는 뜻이다. `roleAssignments/write`를 가진 신원은 자기 자신에게 상위 역할을 부여할 수 있다. AWS는 STS role-chaining으로 특권을 입구 Role 뒤에 감춰 같은 행위의 비용이 다르지만, Azure에는 그 완충층이 없다. 옵트인 변수로 분리하는 것은 같은 문제를 소비자에게 미룬다 |
...
> 추가한다. `modules/aws/*`·`modules/azure/*` 전 모듈에 적용했다.
...
| 잔존물을 **자동으로 지우는** 스크립트 | 만드는 스크립트는 멱등성이 안전망이지만 파기는 아니다. 공용 계정에서 한 번 잘못 돌면 되돌릴 수 없다. 삭제는 사람이 하고 검증만 기계에 맡긴다 |
| 문서를 **경로별로 복제** | 공통부가 두 벌이면 누군가 한쪽만 고친다. 하나로 유지하고 갈림점만 표시한다 |
| `examples/`의 `source`를 **외부 호출 주소**(git URL + 태그)로 | OpenTofu·HashiCorp 표준 모듈 구조는 외부 주소를 쓰라고 적지만, 그러면 CI 게이트 5가 워킹 트리 대신 **릴리스된 태그**를 검증한다. 작업 중인 브랜치가 모듈을 깨뜨려도 예제가 통과하고, 태그 전의 새 모듈은 `init` 자체가 안 되며, 게이트가 네트워크에 의존한다. 그래서 `../..` 상대 경로를 유지한다(`terraform-aws-modules`도 같다). 외부 호출 주소는 루트 `README.md`의 "쓰는 법"과 각 모듈 README의 Usage가 보여준다 |
| 모듈 소싱을 **git 태그 밖으로** 옮기기: 레지스트리 · OCI(`oci://`, digest 고정) · 태그 대신 커밋 SHA | 레지스트리는 저장소 하나에 모듈 하나를 요구해 이 모노레포를 모듈 수만큼 쪼개야 한다. OCI는 불변성이 가장 강하지만 레지스트리 운영·인증·게시 파이프라인이 새로 생기고, 태그 불변성은 태그 ruleset `release-tags`가 훨씬 싸게 준다. SHA 고정은 소비 루트에서 어느 버전인지 읽히지 않고, 얕은 클론(`depth=1`)은 브랜치·태그 ref에만 되므로 클론이 무거워진다 |
| 배포 루트에 **drift 전용 워크플로**(루트를 matrix로 도는 `drift.yml`) | CI의 `backend.hcl`은 `assume_role` 블록을 더 가져 로컬과 모양이 다르고, 그 조립이 `deploy-*.yml`에 있다. 별도 워크플로는 인증 배선의 두 번째 사본이 된다. 드리프트 확인은 `deploy-*.yml`의 `action=plan`과 `changes` 가드가 중복 없이 한다 |
