...
#
# 이 테스트는 git 설치에 조건이 붙지 않는다는 것을 지킨다. git 에 변수를 다시 붙이거나
...
#
# kubeconfig 는 `users[].user.exec` 로 임의 명령을 지정할 수 있다. 그래서 정본이
#    world-writable(0666) 이면 **로컬 권한 상승 경로**가 된다.
...
#
# user_data 는 주석이 본문의 일부다. 어떤 것을 하지 않는다고 설명하는 주석은 그 이름을
