...
컷된 태그는 옛 경로를 그대로 가리키고 지금도 동작한다. 어느 태그부터 전환됐는지는
`git tag -l '<모듈명>-v*' | sort -V`와 `git ls-tree -d --name-only <태그> modules/`로 확인한다.
Azure 모듈은 처음부터 `modules/azure/`에 있어 해당하지 않는다.

옛 경로를 쓰는 소비자가 승급할 때는 태그와 `source` 경로를 같은 커밋에서 올린다. 옛 태그가
계속 동작하므로 승급 시점은 소비 repo가 고른다.
...
**본문에 두 가지를 쓰지 않는다.** 둘 다 이 파일 바깥을 가리키고, 가리키는 쪽이 움직이면
주석은 조용히 틀린 말이 된다. 컴파일도 테스트도 이것을 잡지 못한다.

| 이름 | 무엇 | 왜 쓰지 않는가 |
|------|------|----------------|
| **외부 참조** | 결정 식별자(`D-...`) · 문서 절 번호 · Task·PR 번호 | 가리키는 대상이 움직인다. 절을 하나 넣으면 번호가 밀리는데 주석은 따라오지 않는다. 깨진 링크와 같은 일이 주석에서 일어난다 |
| **이력 서술** | 날짜 · *"실측했다"* · *"여기만 빠져 있었다"* | 읽는 사람의 다음 행동을 바꾸지 않는다. 언제 누가 왜 바꿨는지는 `git blame`과 커밋 메시지가 답한다 |

문서 링크는 **파일 헤더 한 곳**에 문서 단위로만 둔다. 문서 이름은 잘 움직이지 않지만
절 번호는 움직이므로, 가리키려면 절 제목으로 가리킨다.
...
기계로 잡는 것은 3개다. **외부 참조** 2종(절 번호 인용 기호·결정 식별자 `D-...`)과
**이력 서술** 1종(날짜). 주석뿐 아니라 `description` 산문도 본다.
나머지 이력 서술(*"실측했다"* · *"여기만 빠져 있었다"*)은 문맥 판단이라 자동화하지 않는다:
그 판정은 사람이 한다.
