...
| 이 문서가 다루는 범위 | 이 디렉터리(`eks-gitops-hub-spoke/`) 전체가 이 프로파일이다 | 별도 가이드 없음. GitOps가 필요 없다고 판정되면 이 디렉터리의 질문 B·C·D는 적용하지 않는다 |
