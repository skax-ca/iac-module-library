...
# ── 네트워킹: CNI 모드 선택형, 세 모드 전부 이 모듈이 지원한다 ────────────────
...
variable "cni_mode" {
  description = <<-EOT
    ...
    노출하지 않는다):
      ...
      "overlay"     (기본). Azure CNI Overlay + Cilium 데이터플레인
                     ...
                     policy cilium."으로 거부한다). Pod가 VNet 밖 별도 CIDR
      ...
      "node_subnet": Azure CNI(Legacy/Node Subnet). Pod가 node_subnet_id와 **같은**
                     서브넷에서 VNet IP를 받는다(pod_subnet_id는 비워야 한다).
...
variable "pod_subnet_id" {
  description = <<-EOT
    Pod가 속할 서브넷 ID. cni_mode = "pod_subnet"일 때만 지정한다(그 외 모드에서는
    ...
    전 노드 풀(시스템 풀 + var.node_pools 전부)이 이 값 하나를 공유한다. 풀별 오버라이드는
    지원하지 않는다. 서브넷 교체는 노드 풀 순환을 부르므로, 필요해지면 그때 여는 것도
    비파괴 변경이다.
  EOT
  type        = string
  default     = null
...
variable "pod_cidr" {
  description = <<-EOT
    ...
    문서 확인). 그 외 모드에서는 비워야 한다(provider 제약 자체가
...
variable "private_cluster_enabled" {
  description = <<-EOT
    ...
    전제이므로 기본은 private")과 같은 철학을 따른다. ⚠️ false로 여는 것은 옵트아웃으로
    가능하지만, 위 재생성 제약이 그대로 적용된다. 이미 돌고 있는 클러스터에는 무상으로
    되돌릴 수 있는 선택이 아니다.
  EOT
  type        = bool
  default     = true
...
variable "enable_karpenter" {
  description = <<-EOT
    ...
    ⛔ **NAP은 Azure CNI Pod Subnet과 함께 쓸 수 없다**:
    ...
    아직 미해결). cni_mode = "pod_subnet"과는 함께 쓸 수 없다. 아래
    validation이 이 조합을 plan에서 차단한다. cni_mode = "node_subnet" 또는 "overlay"를
    쓴다(cni_mode 변수 설명 참조). cni_mode 기본값이 "overlay"라 기본값
...
variable "web_app_routing" {
  description = <<-EOT
    ...
    ). 그 두 필드는 이 모듈 밖에서 `azapi_update_resource`로 얹어야
