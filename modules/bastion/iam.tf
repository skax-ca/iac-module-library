...
#    그 둘은 "클러스터가 누구를 받아들이는가"이고, 03 §2.3이 "소유 모듈이 허용 소스를
#    변수로 파라미터화한다"고 이미 정했다.
#
...
resource "aws_iam_role_policy" "eks_describe" {
  ...
  # 종속 객체는 약어를 신설하지 않고 부모 이름을 상속한다(카탈로그 규약, 2026-07-30 신설).
...
# ── 인스턴스 프로파일 ─────────────────────────────────────────────────────────
#
# ⚠️ 인스턴스 프로파일 약어는 카탈로그에 **없다**. 임의 생성 금지(CLAUDE.md)이므로
#    「종속 객체는 약어를 새로 만들지 않고 부모 이름을 상속한다」 규약을 적용해 role과 **동일명**을 쓴다.
#    IAM에서 role과 instance profile은 별개 네임스페이스라 충돌하지 않는다(설계 §4.2 상자).
