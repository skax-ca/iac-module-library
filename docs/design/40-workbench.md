...
> ## 🔑 **규칙 — `user_data` 음성 assertion 은 "이름"이 아니라 "행위"를 지목한다**
>
> `user_data` 는 **주석이 본문의 일부**다. 어떤 것을 *하지 않는다* 고 설명하는 주석은
> 그 이름을 반드시 포함하므로, **이름으로 음성 판정을 걸면 설명까지 걸린다.**
>
> **같은 형태가 네 번 나왔다**:
> ① [`30 §4.2`](30-gitops-repo.md) D-ROOTAPP-SKIP **실패 ②** — 마커를 설명하는 주석이 마커로 작동
...
> ⚠️ ③이 별개 교훈을 하나 더 준다: **넓은 음성 판정은 미래의 정당한 요구를 막는다.**
> 금지 대상은 *메커니즘*(사용자별 사본을 무력화하는 export)이지 *장소*(`profile.d`)가 아니었다.
