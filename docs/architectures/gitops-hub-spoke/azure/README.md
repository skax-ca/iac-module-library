...
`aks-platform-gitops`(계층 2)가 갖는다. 그 두 저장소는 설계 근거를 별도 문서가 아니라
`.tf`·매니페스트 **인라인 주석**에 둔다.
...
Virtual WAN 허브는 모듈이 아니라 raw 리소스로 쓴다. 소비자가 허브 하나뿐이라 모듈화의 값
...
계층 판정 원칙은 [../gitops.md](../gitops.md)가 소유한다. AKS에서 그 원칙이 만나는 실물 분류는
AWS와 다르다. **AKS가 컨트롤러까지 관리형으로 가져가는 것이 많아, 계층 2에 Helm 차트가 아니라
CR만 남는 경우가 생긴다.**
...
⚠️ App Routing을 켤 때 레거시 NGINX IngressClass 자동 생성을 명시적으로 끈다. 생략하면 provider
...
토큰은 런타임에 발급된다. AWS에서 스포크 계정이 신뢰 Role을 만들고 허브의 Pod Identity를 신뢰하는
것과 같은 자리다.
...
| `cni_mode` · `pod_cidr` | ❌ | `network_profile` 블록 전체가 ForceNew라 클러스터 재생성. **첫 apply가 사실상 최종 선택이다** |
...
| state 분리 | 배포 루트마다 별도 state(`<env>/<component>.tfstate`). 루트 간 결합은 remote state가 아니라 이름·태그 기반 `data` 조회다 |
| 자격증명 | GitHub OIDC → App Registration 하나. **AWS의 입구 Role → 실행 Role 같은 2단 체인이 Azure에는 없다** |
| 로컬에서 되는 것 | `init`과 `validate`까지다. 배포 루트가 `ci_run` 가드로 로컬 `apply`를 즉시 실패시킨다 |
...
**이 신원에 도달하는 경로를 정확히 하나로 좁히는 것이 유일한 방어선**이다. 경로가 넓어지면
...
| 하지 말 것 | 이유 |
|---|---|
| L7 인그레스로 **Application Gateway for Containers(AGFC)** | frontend가 공인 FQDN만 지원하고 private 옵션이 없다(Microsoft 공식: "Private IP addresses aren't currently supported"). "hub는 전부 private" 원칙과 정면으로 부딪힌다 |
...
| Pod 대역을 **플랫 모델(`pod_subnet`)** 로 두는 것을 기본값으로 | NAP이 Azure CNI Pod Subnet을 지원하지 않고(karpenter-provider-azure#1352), Microsoft 공식 권고도 Overlay를 일반 기본으로 명시한다. 플랫 모델은 Pod 단위 관측성을 절대 포기할 수 없을 때만 고른다([network.md](network.md)) |
...
| CI 신원의 **권한 크기**를 방어선으로 삼기(리소스 그룹 스코프 커스텀 역할 + 불변식 검사) | 한 번 세웠다가 걷어냈다. 배포 루트는 결국 리소스 그룹·역할 할당까지 만들어야 해서 좁힌 역할을 계속 넓히게 되고, 그 과정에서 검사 항목만 늘어난다. AWS 실행 Role도 `AdministratorAccess`라 대칭이 아니었다. **방어선은 권한 크기가 아니라 그 신원에 도달하는 경로(FIC subject) 하나뿐이다** |
