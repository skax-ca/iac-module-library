...
variable "workbench_ami_id" {
  description = <<-EOT
    ...
    🔑 존재하지 않는 ID 를 자리표시자로 두는 것은 **의도된 선택**이다. 그대로 apply 하면 즉시
       실패해 값을 바꿔야 함이 드러난다 — D-EXTDNS-ZONE 의 더미 zone ARN 은 반대로 apply 가
       **성공해서** 존재하지 않는 zone 을 가리키는 IAM 이 조용히 굳는 것이 문제였다(40 §4.1 대비).
  EOT
  type        = string
