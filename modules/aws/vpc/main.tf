...
locals {
  ...
  # slice 상한을 min()으로 자르는 것은 리전 AZ가 부족할 때 locals가 index 오류로 죽는 대신
  # aws_vpc의 precondition이 도메인 메시지로 보고하게 하기 위해서다.
  ...
  # 존재하지 않는 suffix는 null로 두고 precondition이 보고한다.
  ...
  subnet_defs = local.enabled ? {
    for def in flatten([
      for group_name, group in var.subnet_groups : [
        # cidrs를 az_suffixes 범위로 잘라낸다. 초과분은 precondition이 보고한다.
  ...
  # az_suffixes 순서를 써야 single NAT가 "첫 AZ"에 놓인다.
...
data "aws_availability_zones" "this" {
  ...
  # suffix 1글자 매칭이 AZ 이름을 "<region><letter>" 형태로 전제하기 때문이다.
  filter {
    name   = "opt-in-status"
...
resource "aws_vpc" "this" {
  ...
  # EKS 프라이빗 엔드포인트·VPC 엔드포인트 프라이빗 DNS가 둘 다 요구한다.
  # provider 기본값은 hostnames = false라 명시하지 않으면 동작하지 않는다.
  ...
  lifecycle {
    # OpenTofu 1.12부터 prevent_destroy가 입력 변수를 참조할 수 있다.
    ...
    # 그룹의 AZ 수는 cidrs 길이가 결정한다. 잘못된 상태를 표현할 수 없는 인터페이스의 전제 조건.
    precondition {
      condition = alltrue([
        for name, group in var.subnet_groups : length(group.cidrs) >= 2 && length(group.cidrs) <= var.az_count
      ])
      error_message = "subnet_groups의 각 그룹은 cidrs를 2개 이상 az_count(${var.az_count})개 이하로 가져야 한다(cidrs 길이가 곧 그 그룹의 AZ 수다)."
    }

    # NAT는 public 그룹의 서브넷에 놓이므로 public 그룹 없이는 만들 수 없다.
    precondition {
    ...
    # per-AZ NAT 모드에서 커버되지 않는 AZ가 생기면 그 AZ의 private 서브넷에 기본 경로가 없다.
    # AZ는 항상 az_selection 앞에서부터 배정되므로 AZ 집합은 접두사 관계다 → 개수 비교로 충분하다.
    precondition {
      condition     = !local.nat_enabled || var.single_nat_gateway || length(local.nat_host_suffixes) >= local.max_private_az_count
      error_message = "per-AZ NAT(single_nat_gateway = false)에서는 NAT 호스트 그룹('${coalesce(local.nat_group, "-")}')의 AZ 수가 private 그룹의 최대 AZ 수(${local.max_private_az_count}) 이상이어야 한다."
    }
  }
}
...
resource "aws_subnet" "this" {
  ...
  # map_public_ip_on_launch를 설정하지 않는다. public 그룹의 용도는 ELB·NAT 호스팅이고
  # 둘 다 자동 공개 IP가 불필요하다(ALB는 자체 주소, NAT는 EIP).

  tags = merge(
    var.tags,
    each.value.extra_tags,
    # EKS 태그는 eks_role이 지정된 그룹에만 붙인다. public/private 전체에 일괄 부착하지 않는다.
...
# private 그룹은 AZ별 RT. NAT 경로가 AZ마다 갈리기 때문이다.
resource "aws_route_table" "private" {
  for_each = { for key, def in local.subnet_defs : key => def if def.type == "private" }

  vpc_id = aws_vpc.this[0].id

  # each.key가 "<group>-<az suffix>" 이므로 private RT의 Name 패턴과 그대로 일치한다.
  tags = merge(var.tags, {
    Name = "rtb-${local.name_mid}-${each.key}"
  })
}

# ⛔ 구조 라우트를 aws_route_table의 inline route 블록으로 쓰지 않는다.
#    inline route와 aws_route는 혼용 불가다(provider가 서로의 규칙을 덮어쓴다).
#    이 모듈은 소비자가 이 RT에 aws_route로 운영 라우트를 얹는 것을 전제한다.
...
resource "aws_route" "private_nat" {
  # 커버되지 않는 AZ는 여기서 제외하고 aws_vpc의 precondition이 보고하게 한다.
  for_each = {
