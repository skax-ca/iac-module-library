# workbench 출력 계약 — 설계 docs/design/40-workbench.md §4.4
#
# ⭐ 아래 둘(security_group_id · iam_role_arn)이 D-WORKBENCH-SEAM의 실물이다.
#    이 모듈은 EKS 접근 2·3층을 만들지 않고 **재료만 내보낸다** — 조립은 소비 루트가 하고,
#    그 형태는 examples/workbench-enterprise가 보여준다(설계 §5).
#
...
output "workbench_security_group_id" {
  description = <<-EOT
    workbench SG. **eks-cluster의 cluster SG 추가 규칙 소스로 넘긴다**(설계 §5, 3층).
...
output "workbench_iam_role_arn" {
  description = <<-EOT
    workbench role ARN. **eks-cluster의 access_entries principal로 넘긴다**(설계 §5, 2층).
