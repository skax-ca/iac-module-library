...
locals {
  ...
  managed_node_groups = {
    for ng_key, ng in var.managed_node_groups : ng_key => {
      ...
      # (기본 AL2023_x86_64_STANDARD)이고 루트가 each.value.ami_type을 **그대로 넘긴다**.
...
module "eks" {
  ...
  # 한편 AWS는 public이 꺼진 상태에서 publicAccessCidrs 변경을 **반영하지 않는다**.
...
module "karpenter" {
  source = "terraform-aws-modules/eks/aws//modules/karpenter" # ⚠️ registry 주소에 /aws 필수
  ...
  # 관리형 정책은 6,144자 한도(조정 불가)를 초과해 LimitExceeded가 난다(upstream #3563).
...
#    오브젝트로 라벨·taint를 알리므로 이 태그는 읽히지 않는다. 어떤 노드그룹이 "지금"
