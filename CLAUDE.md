...
**문체 규칙**은 `docs/writing-style.md` 2절이 소유하고, **산문을 쓰는 모든 작업에 걸린다**:
문서·`.tf` 주석·`variables.tf` description·GitOps 매니페스트 주석·셸 `echo` 문구. 파일 확장자로
가르지 않는다. 8개 규칙 전부 사람이 리뷰 시 적용한다(기계 게이트가 잡는 것은 문체가 아니라
좌표다). ⛔ 문장부호를 금지 목록으로 만들지 않는다.
⛔ 커밋·태그 메시지는 범위 밖이다(사건 서술이 본업이다).
