...
세 모듈이 있고, 순서대로 의존한다. `vpc` -> `eks-cluster` -> `workbench`.
...
예: `vpc-acme-prd-an2-main` · `eks-acme-prd-an2-main-01`

리소스 약어는 [`reference/aws-naming-abbreviations.md`](reference/aws-naming-abbreviations.md)가 소유한다.
