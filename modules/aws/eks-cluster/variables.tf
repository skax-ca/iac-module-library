...
variable "deletion_protection" {
  description = <<-EOT
    ...
    cluster_enabled = false. 보호를 켠 채로는 한 번에 파기할 수 없다.
...
variable "public_access_cidrs" {
  description = <<-EOT
    ...
    ⚠️ 빈 리스트를 넘기면 EKS가 0.0.0.0/0으로 기본 적용한다. public을 켤 때는 좁힌다.

    ⚠️ **endpoint_public_access = false면 이 값은 무시된다**: 모듈이 upstream에 null을 넘기기
...
variable "managed_node_groups" {
  description = <<-EOT
    ...
    ⚠️ **instance_types와 아키텍처가 일치해야 한다.** graviton(t4g·m7g·c7g…)을 쓰려면
...
variable "external_dns_hosted_zone_arns" {
  description = <<-EOT
    ...
    그 정책을 만든다. prd만의 권고가 아니다. **이 조건이 어긋나면 어느 환경이든 apply가 막힌다.**
  EOT
  type        = list(string)
  default     = []
  nullable    = false
...
variable "argocd_namespace" {
  description = <<-EOT
    ...
    ⚠️ argocd-seed.sh(`eks-reference-infra`)의 ARGOCD_NAMESPACE와 일치해야 한다. 어긋나면
