# Notepad MANUAL Archive — iac-module-library

> 자동 로드되지 않는 영구 보관 아카이브. `.omc/notepad.md`의 Working Memory에서
> 완료된 옛 세션 서술을 여기로 이관한다. 필요할 때만 Read/grep으로 직접 조회한다
> (2026-08-30 분리 — 별도 파일이라 더 이상 notepad.md 총 바이트에 안 잡힌다,
> `.omc/notepad.md` Priority Context 참조).
...
### 2026-08-25 05:20
2026-08-25 (이어서, 새 세션) — docs/ 전체 9개 문서를 "처음 보는 팀원이 이 저장소 문서만 보고 이해할 수 있는가" 기준으로 순차 리뷰·수정 완료(conventions.md → decisions.md → team-access.md → module-index.md → architectures/README.md·aws-naming-abbreviations.md → overview.md·choose-your-path.md). PR #31 이후의 후속 세션.

주요 변경(전부 main에 커밋·push 완료, PR #32 하나만 브랜치 경유):
① conventions.md 8·9절(문서 작성 규칙·문체 규칙)을 신규 docs/writing-style.md로 분리 — conventions.md의 실제 독자("코드를 쓰거나 모듈을 소비하는 사람")와 8·9절의 독자("문서를 쓰는 사람")가 달라 8절 규칙 1번("독자로 파일을 가른다") 자체를 conventions.md가 어기고 있었음. 같은 세션에 TGW 각주·stop-slop 채택 서술 등 날짜·사건 서술 제거, "결정 식별자(D-...)" 하위 절 전체 삭제(이 저장소는 그 메커니즘을 쓰지 않기로 확정).
② 파일명이 번호 접두사 없이 이미 바뀐 뒤(PR #31)에도 H1 제목에 옛 번호(00/01/02/05/06/08)가 남아있던 결함을 6개 파일에서 일괄 정리.
③ team-access.md: decisions.md를 인용하던 문구가 실제로 그 파일에 없는 죽은 참조였음 → 제거. GitHub Actions 무료 사용량(private repo 월 2,000분, public 무제한) 사용자 요청으로 추가.
④ module-index.md → module-catalog.md 이름 변경(PR #32, squash merge, `.tf` 37곳 참조 포함 — 헤더 주석 "# 계약: docs/module-index.md" 등). 이름 변경 전에 내용도 재구성: 모듈별 입력/출력 표가 각 모듈 README(terraform-docs 자동생성, CI drift 검사 대상)와 이중 관리되고 있어 제거하고 링크로 대체(304→168줄, 이후 165줄). workbench 모듈 고유 정보(IAM 권한·부팅 후 상태)는 그 모듈 README로 이전, 중복이던 "실행 기반" 표는 overview.md로 통합. 신규 모듈 추가 시 이 파일에 필요한 건 목록 한 줄 + 배선 설명뿐이라 향후 비대화 우려 해소.
⑤ architectures/README.md·aws-naming-abbreviations.md: writing-style.md 규칙 1번(첫 줄 "읽는 사람")을 이 두 파일만 안 지키고 있었음(기계 검증 대상 아니라 여태 안 걸림) → 추가.
⑥ overview.md·choose-your-path.md 정밀 리뷰(이전 세션들은 단편 참고만 했었음, 이번에 처음부터 끝까지 봄):
  - choose-your-path.md의 TGW/RAM 설계 서술에 섞여있던 "처음엔 X 시도, 날짜에 Y로 실패, Z로 바꿈"류 사건 서술 10곳을 decisions.md 신규 「크로스 계정 네트워킹 (허브-스포크 TGW)」 카테고리로 이관, choose-your-path.md엔 결론+링크만 남김(400→337줄, 더 이상 400줄 상한 안 걸림).
  - 문서 간 §N/N절/N행 인용 금지(writing-style 규칙6) 위반 4곳을 실제 링크 또는 제목 인용으로 수정.
  - "질문 A" 절에 "프로파일 A(GitOps)/B(직접 배포)" 이름만 있고 실제 정의가 전무했던 걸 발견(사용자 지적) → 비교표 추가. 1차 작성 후 재검토 요청받아 자체 검증: 이 저장소 모듈 4개(vpc/eks-cluster/workbench/cross-account-trust-role) 전수 grep으로 helm_release 리소스가 하나도 없음을 확인 → "프로파일 B는 CI가 직접 적용한다"는 1차 서술이 근거 없는 추정이었음을 스스로 발견해 "이 저장소는 방법을 규정하지 않는다"로 정정. 워크로드(계층3)는 두 프로파일 모두 범위 밖이라는 것도 overview.md 3계층 표와 대조해 바로잡음.
  - overview.md("팀에 처음 온 사람" 독자) 자체도 재검토: "GitOps"라는 핵심 용어가 저장소 어디에도 정의돼 있지 않았음 → push/pull 정의 추가. `eks-reference-infra`가 정의 전에 먼저 등장하던 순서 문제 수정. "ALBC" 약어 첫 등장에 풀어씀.
  - 사용자가 "섹션 3~4가 choose-your-path.md 성격 아니냐"고 재질문 → 확인해보니 실제로 choose-your-path.md 질문 C("addon을 어디에 두는가")와 같은 분류를 다른 각도로 중복 서술 중이었음(단순 고도 불일치가 아니라 진짜 중복). 섹션 3은 원칙 한 문단만 남기고 질문 C로 링크, 섹션 4(판별 두 가지+CR 예시 5개 표) 전체를 choose-your-path.md 질문 C의 신규 하위 절 「설정 CR이 계층 2인가 3인가」로 완전 이관. overview.md 섹션 번호 5·6·7→4·5·6 재조정, 이관 과정에서 깨질 뻔한 자기참조(질문 D 표가 옛 섹션4를 가리키던 링크)도 문서 내부 제목 참조로 수정.
  - 사용자가 "3계층 소유 모델 표 1행(인프라)의 '어디에'를 eks-reference-infra로 바꿔야 한다"고 지적(2행 eks-platform-gitops는 이미 구체명인데 1행만 `<project>-infra` 범용 표기였던 비일관). 표+섹션4 다이어그램 둘 다 수정, ASCII 다이어그램 세로 정렬(글자수 차이로 깨졌던 것)도 정밀 재정렬.

검증: 매 수정마다 python3 scripts/validate-doc-conventions.py(19개 파일) 전체 재실행해 통과 확인. 문서 전용 변경이라 CLAUDE.md 브랜치 규칙상 전부 main 직접 커밋(module-catalog.md 이름 변경만 .tf 37곳이 걸려 브랜치→PR #32, 로컬 게이트 전부 재검증 후 squash merge).

세션 종료 시점 상태: docs/ 9개 문서(+신규 writing-style.md) 전부 이 기준으로 리뷰 완료. 다음 세션에서 더 볼 문서 없음(전수 완료) — 새 리뷰 요청이 없으면 이 라운드는 종결된 것으로 본다.
### 2026-08-26 00:28
### 2026-08-26 — Azure 기반 구조 ralplan 프로세스 통찰

3회 반복(Planner→Architect→Critic 독립 검토, 비교 없이 각자 같은 스냅샷 평가) 방식이 이번에 실제로 값을 냈다: 1차는 설계 판단 결함(축3/미결 모순, P5 위반 등) 위주였지만, 2·3차로 갈수록 결함이 "계획 자신의 검증 장치"(grep이 실은 이 세션의 ugrep shim이라 결과가 갈림, 약어 카탈로그 검증 스크립트의 정규식이 실제 문서 형식과 안 맞아 상단 총계 검사가 죽어 있었음, `find -mindepth/-maxdepth`가 `-o` 절마다 재적용 안 되는 셸 버그) 층으로 이동했다. 두 검토자가 완전히 독립적으로 같은 결함에 도달하는 빈도가 신뢰도의 실제 신호였다. 3차에서 둘 다 "더 반복해도 이익보다 비용이 크다"고 명시적으로 판단해 멈췄다 — 다음에 비슷한 규모(브레이킹 체인지 + 다수 CI 게이트 동시 수정)의 ralplan을 돌릴 때, 결함 성격이 "설계"에서 "검증 장치"로 옮겨가면 수확체감 신호로 볼 것.

실행(team) 단계에서도 executor 3명(PR1·PR2·Step5)이 계획 밖 결함을 각자 발견했다(깨진 상대링크, `.opencode/` stale 참조, 계획 자체의 B6 인수조건 함정 — staged 파일이 HEAD와 동일하면 위양성). 전부 임의 판단 없이 보고 후 승인받고 처리한 패턴이 잘 작동했다 — 다음에도 이 패턴(발견 시 즉시 처리하되 스코프 확장은 보고·승인 후) 유지.
### 2026-08-26 08:00
### 2026-08-26 (이어서) — Azure vnet 모듈 설계 완료 및 실행 (ralplan 4회 반복, main 직접 커밋)

Azure 기반 구조(약어 카탈로그 분리·태깅 절 중립화·모듈 경로 재편·.claude/rules 중립화) 완료 후 첫 Azure 모듈 `vnet` 설계 착수. `/oh-my-claudecode:ralplan`로 Planner→Architect→Critic 4회 반복(1회차 Critic ITERATE MAJOR7 · 2회차 둘 다 CRITICAL 독립 발견("빈 NSG=no-op" 거짓) · 3회차 둘 다 CRITICAL 독립 발견(v2 정정 자체의 거짓 "공용 LB 백엔드 끊김"·Bastion이 만들 수 없는 예약이름 서브넷) · 4회차 둘 다 "설계 재검토 불필요, 인라인 수정으로 충분" 독립 판정). 세 라운드 연속 같은 실패 패턴(검증 안 된 사실을 근거로 씀)이 재발하다 4회차에서 사실관계는 전부 정확해지고 결함 성격이 "목록 완결성·인수조건 커버리지"로 이동 — 수확체감 신호로 라운드 종료.

4회차 결과(Architect 조건부승인 5건 권고 + Critic ITERATE-인라인 MAJOR5건: NAT StandardV2가 preview임을 놓침·예약이름 서브넷 목록에 AzureFirewallSubnet 누락·0-6 델타표가 AppGw 데이터플레인 트래픽 델타 누락·인수조건 커버리지 구멍 2건)을 team-lead(나)가 직접 계획 파일에 병합(v5, Planner가 주간 세션 한도로 사용 불가). 사용자에게 G1(Flow Logs 제외)·G2(NSG 옵트인 기본false) 확정받고 즉시 Step1~5 실행 승인받아 같은 세션에서 완주.

**실행 결과**(각 Step 인수조건 전부 실행 검증, main 직접 커밋 예정):
- Step1: `docs/naming/abbreviations/azure.md` 신설(6개 약어: vnet·snet·nsg·rt·ng·pip, CAF 권위소스로 AWS 물리ID접두사 규칙 1항 대체). `docs/README.md` 데이터 표에 행 추가.
- Step2: `docs/conventions.md` §2 "Name 태그 포맷"→"리소스 이름 포맷"로 provider 중립화(공통 강제방식 4곳: 절제목·1번·2번링크·4번), Azure 강제방식에 신규 4번(name 인자, Name태그 안닮)·5번(제약 리소스 표: virtualNetworks 등 길이/스코프)·기존 정책미결 6번으로 재배치, 리전코드에 krc 추가.
- Step3: `docs/module-catalog.md`에 `vnet` 배선절 신설(만드는것/안만드는것·예약이름서브넷 3종 한계 명시·출력비대칭 3건 표·인터페이스 초안 HCL·계약 5건).
- Step4: `docs/decisions.md`에 「Azure 네트워킹 (vnet)」 ADR 평면 `##` 절로 「모듈 설계」 다음 형제절 삽입(기각안 10개 표 + 결정/파급 인용구, 기존 문서 스타일과 일치하도록 재작성).
- Step5: `open-questions.md` G1·G2 해소 처리 + 직전(azure-foundation) 라운드 Follow-up 5번 해소·6번 부분해소·2번4번 승격 기록.

⚠️ 실행 중 발견한 함정 3건(계획에 없던 것, 인수조건 실행이 즉시 잡음): (1) azure.md에 "conventions.md §2" 식 절 인용이 writing-style.md의 §인용금지 규칙 위반 → 링크/제목인용으로 정정 (2) 새로 쓴 문장 2곳(conventions.md·module-catalog.md)에 em-dash(—) 혼입 → 마침표로 정정 (3) ADR 작성 시 기존 decisions.md 스타일(표 먼저, `> **결정**:` 인용구는 표 뒤)과 다르게 "결정" 단락을 표 앞에 배치했다가 스타일 불일치 발견, 재작성해 통일.

⚠️ 미처리(범위 밖, 보고만 함): `open-questions.md`의 "azurerm provider의 태그 상속 메커니즘 확인" 미결 항목(2026-08-25 azure-foundation 라운드 잔존)이 실제로는 conventions.md Azure강제방식 2번에 이미 해소되어 있는데 미결로 남아있는 stale 항목 발견 — 이번 라운드 스코프 밖이라 손대지 않음, 다음 세션에서 정리 필요.

다음 세션: (1) 이 커밋 확인 (2) 구현 라운드 착수 게이트 4건(tflint azurerm 등록 등) 처리 여부 판단 (3) 실제 `.tf` 구현(modules/azure/vnet/) 착수 여부 — 별도 브랜치→PR 필요(CLAUDE.md 브랜치 규칙) (4) 위 stale 미결 항목 정리.
### 2026-08-26 08:41
### 2026-08-26 (이어서) — Azure vnet 모듈 실제 구현 (브랜치 `feat/azure-vnet-module`)

설계(같은 날 앞서 완료, ADR·인터페이스 초안 확정)를 실제 `.tf`로 구현. 계획 `.omc/plans/2026-08-26-azure-vnet-design.md`의 축1~5b 결정과 `docs/module-catalog.md` 인터페이스 초안을 그대로 따름. `modules/aws/vpc`의 코드 패턴(파일 헤더·precondition 스타일·null-safe 출력·주석 규칙)을 이식.

커밋 2개:
1. **구현 라운드 착수 게이트 4건** — `.tflint.hcl`에 azurerm ruleset 정확 핀(`0.32.0`) 등록, `.githooks/pre-commit`에 `tflint --init` 단계 추가(GITHUB_TOKEN rate limit 대응), pre-commit stale 태그 검사 컴포넌트 목록에 `vnet` 추가, `docs/conventions.md` 핀 표에 azurerm provider 하한(`>= 5.0`)·ruleset 핀 행 추가.
2. **`modules/azure/vnet/` 신규 구현** — `versions.tf`·`variables.tf`·`main.tf`·`outputs.tf`·`tests/plan.tftest.hcl`(11개 시나리오)·`examples/basic/`·README(terraform-docs 생성). 서브넷은 `azurerm_subnet` 별도 리소스(인라인 subnet/dns_servers 미사용), NSG·라우팅 테이블은 그룹별 옵트인(룰·라우트는 소비자가 별도 리소스로 얹는다, inline 혼용 금지 원칙 코드 주석에 명시), NAT는 수요(`nat_routed=true`) 0개면 조용히 스킵, `deletion_protection`/`vnet_enabled` 교차검증, `nat_gateway_sku_name`(기본 Standard)/`nat_gateway_zones`(StandardV2일 때 zones 금지) 교차검증, 예약 이름 서브넷 미생성.

로컬 검증 전부 실측 통과: `tofu fmt -check`(0) · `tofu test`(11 passed, 0 failed) · `tflint --recursive`(azurerm ruleset 포함, 0) · `trivy config`(CI와 동일 커맨드로 전체 재검증, misconfig 0) · 예제 `tofu validate`(Success) · `terraform-docs` drift 검사(README 재생성 후 diff 없음, CI 핀 v0.24.0과 동일 버전으로 실측).

⚠️ 이번 라운드에서 실측으로 확인해 코드 주석에 반영한 것: `azurerm_public_ip`의 `sku`가 실제로 `StandardV2`를 지원함(공식 문서 재확인 완료, 처음엔 의심했으나 반증됨) — NAT Gateway와 Public IP의 SKU를 같은 값으로 맞추는 설계가 유효함을 확인.

다음: PR 오픈 완료(본문에 검증 체크리스트 포함) → CI 통과 확인 → merge 여부·`vnet-v0.1.0` 태그 컷 여부는 사용자 결정 대상.
### 2026-08-27 00:14
### 2026-08-27 — nullable = false 크로스컷 (브랜치 feat/nullable-false-crosscut → PR #36 squash merge)

세션 시작 시 사용자가 제안한 두 후속 과제(project-memory open-items) 중 1·2번 진행.

**2번(먼저 처리)**: `.omc/plans/open-questions.md`의 "azurerm 태그 상속 메커니즘 확인" 미결 항목이 stale임을 확인(실제로는 `docs/conventions.md` Azure 강제 방식 2번이 공식 문서 인용까지 붙여 이미 해소했음) → "해소된 항목"으로 이동. 이 파일은 `.gitignore:50`의 `/.omc/*` 대상이라 git 이력 없음, 편집만으로 충분.

**1번**: `nullable = false` 크로스컷. CLAUDE.md 설계 우선 원칙(인터페이스 변경 예외 없음)에 따라 구현 전 설계 확인 절차를 거쳤다.
- OpenTofu 공식 문서(WebFetch)로 `nullable=false`+`default` 상호작용 검증: 명시적 `null` → default로 대체. `default=null` 자체와 조합하면 모순(대체해도 다시 null).
- aws-ia standards-terraform(WebSearch+WebFetch)도 같은 패턴 권고("필수 변수엔 nullable=false 고려") 확인 — project-memory가 인용한 "AWS 공식 팀 표준"의 실제 근거.
- 실제 크래시 근거를 grep으로 실측: `modules/aws/vpc/main.tf`에 `merge(var.tags, {...})` 7곳, `modules/azure/vnet/main.tf`에 2곳 — `tags = null` 명시 시 "argument must not be null" 크래시.
- 판정 기준 설계(적용: default 없음(필수) 또는 default가 null이 아닌 변수 / 제외: default 자체가 null이고 그 null이 의도된 값인 변수)를 AskUserQuestion으로 확인받은 후 구현.
- 대상: vpc 17개 중 14개(제외 3: az_selection·eks_cluster_name·flow_logs_kms_key_id), vnet 12개 중 9개(제외 1: nat_gateway_zones).
- 문서화: `docs/decisions.md`에 「변수 계약 (nullable)」 ADR 신설(하지 말 것 3항 + 결정/파급), `.claude/rules/terraform.md`에 신규 변수 작성 시 판정 기준 codify(향후 drift 방지).
- 테스트: 각 모듈에 `nullable_false_falls_back_to_default` run 추가 — `tags=null`·`purpose=null`·`vpc_enabled/vnet_enabled=null`을 명시적으로 넘겨 plan이 크래시 없이 default로 대체됨을 실제로 증명(기존 테스트는 "안 깨졌다"만 보여주지 "고쳐졌다"는 못 보여줌).

검증: `tofu fmt` 전체·`tflint --recursive` 양쪽·`trivy config` 양쪽(misconfig 0)·`tofu test` vpc 14/14·vnet 12/12·pre-push 훅이 전체 5개 모듈 78개 테스트 재실행(무영향 확인)·예제 2개 `validate`·terraform-docs drift(README 무변경 — nullable은 렌더 컬럼 아님)·`validate-doc-conventions.py`.

PR #36: CI 3게이트(문서 작성 규칙·약어 카탈로그 SSOT·공통 검증) 전부 pass 확인 후 사용자 확인받아 squash merge(`34a2e2d`), 로컬·원격 브랜치 정리(`git fetch --prune`로 스테일 원격 추적 브랜치 18개 일괄 정리됨 — 전부 이미 삭제된 과거 PR 브랜치).

**다음 세션**: `vpc-v0.5.0`·`vnet-v0.2.0` 태그 컷 여부/시점을 사용자에게 확인할 것(사용자가 이번 세션엔 "merge만, 태그는 나중에"로 명시적으로 미룸 — 태그 없이 이 변경은 소비 repo에서 아직 참조 불가 상태).
### 2026-08-28 03:49
### 2026-08-28 — Azure aks-cluster 모듈 설계 (ralplan, v2→v4, 계획 문서만 — main 미커밋)

Azure 두 번째 모듈(aks-cluster) 설계를 ralplan(Planner→Architect→Critic)으로 완결. 소비 repo `aks-reference-infra`(로컬 `/Users/a07326/born2k/ai/aks-reference-infra`)의 실제 커밋(`0100235`, CNI Pod Subnet 확정)·`CLAUDE.md`(CI 신원 `roleAssignments/write` 금지 불변식)를 실물 대조하며 진행.

핵심 결정: (1) 사용자 지시로 모듈은 identity·role assignment를 안 만들고 `identity_id`만 입력받음 — AWS의 `cross-account-trust-role`/`eks-cluster` 분리 패턴을 Azure에 적용, 근거는 "같은 철학(신원이 특권을 직접 안 가짐), 다른 메커니즘"(AWS=STS 체인, Azure=대응 개념 없음) (2) CNI는 Azure CNI Pod Subnet(flat) 고정, Overlay 미노출(G-A 확정) (3) Pod 서브넷은 `vnet` 모듈 소비자가 만들고 ID만 입력 (4) 노드 풀 이름은 Azure 물리 제약(하이픈 불가·12자 한도)으로 약어 카탈로그가 아니라 `conventions.md` 소유로 신설 판정 (5) Entra RBAC는 옵트인+로컬계정 유지(G2 확정).

라운드별: v2 Architect 조건부승인·Critic ITERATE(CRITICAL 1 — `validate-abbreviations.py`가 `npsystem` 8자 초과+예시 하이픈 요구로 rc=1 실증, MAJOR 7). v3에서 전부 해소(축1b/1c 신설로 CRITICAL 해소, 0-16-3 라우팅테이블 미결도 Microsoft Learn 원문 재조회로 완전 해소, "커스텀 역할로 좁히면 오히려 금지 권한을 부르는 함정" 신규 발견) → Architect 승인, Critic ITERATE(승인 직전, 6절 ADR 초안이 본문이 반증·금지한 내용을 그대로 담은 문안 3건+숫자 2건만 잔존). Planner가 세션 한도(리셋 13:40)로 마무리 직전 중단 → team-lead가 Critic이 이미 작성해둔 정확한 교체 문장으로 직접 5건 반영해 v4로 마감(vnet 라운드와 동일한 처리 패턴, project-memory `ralplan-ops` 참고).

사용자가 G-A·G2 게이트 둘 다 계획의 권고안대로 확정. 계획서: `.omc/plans/2026-08-28-azure-aks-cluster-design.md`(1,367줄, v4, gitignore 대상 — git 이력 없음, 다른 머신에서 이어가려면 project-memory가 유일한 근거). 실행(Step1~5: `docs/naming/abbreviations/azure.md`·`conventions.md`·`module-catalog.md`·`decisions.md` 갱신 + `.omc/` 정리, main 직접 커밋)은 사용자가 다음에 직접 진행하기로 결정, 이번 세션엔 미착수. `.tf` 구현은 별도 브랜치.
### 2026-08-28 04:05
### 2026-08-28 (이어서) — Azure aks-cluster 모듈 설계 Step1~5 실행 (main 직접 커밋 예정)

계획 `.omc/plans/2026-08-28-azure-aks-cluster-design.md`(v4, 같은 날 앞서 승인)의 실행 라운드.
세션 시작 시 사용자에게 남은 게이트 2건(G-A·G2)을 AskUserQuestion으로 확인 — 둘 다 계획의
권고안대로 확정(G-A: Overlay 미노출·flat 고정, G2: Entra 옵트인·로컬 계정 유지). 즉시 Step1~5
착수 승인받아 같은 세션에서 완주.

**Step1** `docs/naming/abbreviations/azure.md` — `aks`(A.5 Containers 신설)·`id`(A.4 Identity
2번째) 등재. 노드 풀 약어(`npsystem`/`np`)는 등재하지 않음(등재 규칙 4의 7자 상한 + 예시 형식
검사를 동시에 위반, 계획 0-27이 이미 실증). 총계 9→11개, 4→5개 카테고리.

**Step2** `docs/conventions.md` — §2 "조합 방식과 구성 요소는 클라우드와 무관하게 같다" 단정을
"리소스 그룹·구독 스코프에서는 같다"로 한정하고, Azure 강제 방식에 6번(하위 스코프 리소스 예외)
신설 — 노드 풀 이름 형태(`npsystem`/`np<그룹키>`)·물리 제약(1~12자·소문자+숫자)·길이 예산
(그룹 키 8자 이하, `temporary_name_for_rotation`이 같은 12자 한도를 쓰기 때문)·Windows 미지원을
표로 실었다.

**Step3** `docs/module-catalog.md` — `vnet` 절 뒤에 `aks-cluster` 절 신설(설계 확정·구현
미착수 명시). `vnet`과의 연동(노드/Pod 서브넷·아웃바운드·라우팅 테이블 불필요·서브넷 위임
금지·NSG·예약 CIDR·신원 순서 의존) + 「인터페이스 초안」 서브섹션(README 생기면 삭제 예정,
필수입력 6개·선택입력 15개·출력 6개) + `eks-cluster`와의 비대칭 표(11축) 등재.

**Step4** `docs/decisions.md` — 「Azure 네트워킹 (vnet)」 절 뒤에 「Azure 컨테이너
(aks-cluster)」 평면 절 신설. 기각 표 25행(노드 풀 이름 대안 3·신원 대안 4·Entra 대안 2·
CNI 대안 2·노드 풀 검증 대안 3·기타 11) + 결정/드라이버/근거 인용구 + 파급 표 9행. 「되살리면
안 되는 근거」에 2행 추가(vnet AVM 기각 근거 재인용 금지, eks-cluster IAM 선례 오용 금지).

**Step5** `.omc/plans/open-questions.md`에 이 라운드 절 append — G-A·G2 확정 기록 + 실행
완료 요약 + 미결 9건(구현 라운드/v0.2.0/조사 항목으로 차단 범위 구분) 이월.

⚠️ 실행 중 계획에 없던 함정 2건, 인수 조건 실행이 즉시 잡음: (1) `azure.md` 개정 이력에 노드
풀 미등재 사유를 적으며 리터럴 `npsystem` 문자열을 실수로 남겨 음성 가드(A3b, "이 문자열이
카탈로그에 없어야 한다")를 위반 — "CAF 권장 시스템/사용자 노드 풀 약어(8자)" 식 우회 표현으로
정정. (2) 새로 쓴 문장 6곳에 em-dash(—) 혼입(azure.md 2·conventions.md 2·module-catalog.md 3)
→ 전부 마침표·괄호로 정정. 둘 다 `validate-doc-conventions.py`/`validate-abbreviations.py`
재실행으로 즉시 검출.

모든 Step 인수 조건 + `validate-doc-conventions.py`(22개 파일)·`validate-abbreviations.py`
전부 실측 통과. 다음: 이 4개 문서 + `.omc/notepad.md`를 `main`에 직접 커밋(문서 전용,
`.tf` 0건 확인 완료) → 사용자 확인 후 push. 구현(`.tf`, `modules/azure/aks-cluster/`)은
별도 브랜치 착수 대상 — 이번 라운드 스코프 밖.
### 2026-08-28 05:58
### 2026-08-28 (세션 3, 이어서) — Azure aks-cluster `.tf` 구현 + Karpenter(NAP) 채택

이전 세션(Step1-5, docs 확정)에 이어 같은 날 진행. 사용자가 "남은 것들 순서대로 진행하자"로 시작 → 구현 브랜치 `feat/azure-aks-cluster-module` 생성 → 게이트 실측(node_provisioning_profile 자기모순 확인) → 모듈 전체 구현(versions.tf·variables.tf·main.tf·outputs.tf·tests 22개·examples/basic·README) → trivy MEDIUM+ 5건 해소(4건 코드로, 1건 trivyignore) → 커밋(e7385c1).

사용자가 EKS의 GitOps 패턴과 비교 요청 → argocd_hub_pod_identity 코드 실측(iam.tf) → "AKS에서 node_pool=EKS 노드그룹 맞아? Karpenter는?" 질문 → 리서치 결과 mode="Manual" 하드코딩(설계 밖 구현 판단)임을 밝힘 → 사용자 "eks와 동일하게 karpenter 기반으로 갈거야, 당장 수정해서 테스트" → CNI 조합(flat vs Overlay+Cilium) 미검증 리스크를 AskUserQuestion으로 먼저 확인받음(flat 유지 선택) → enable_karpenter 변수 추가, mode=Auto 기본(0e5607e).

⚠️ 이 라운드에서 검색 도구 신뢰도 사고 1건: WebSearch 요약이 만든 "only supports and is tested with Overlay+Cilium"이라는 문구를 1차 출처 인용처럼 썼다가, 사용자가 "제약이면 공식 페이지에 적혀 있었을 것"이라고 반박 → 원문(github.com/Azure/karpenter-provider-azure README) 직접 fetch해 그 문구가 없음을 확인, "Known limitations"엔 Pod Subnet 배제가 없다는 것으로 정정. 교훈: WebSearch 요약문은 인용 근거로 못 쓴다, 항상 원문까지 fetch해야 한다.

이어서 "managed karpenter 비용이 진짜 무료야? tier 차이는? GitOps로 NodePool 정의하는 패턴 문제없어?" 질문 → 정밀 리서치(가격 페이지·free-standard-pricing-tiers·node-auto-provisioning-node-pools 등) → default_node_pools="None" 미반영 gap 발견 → 사용자 "추가해줘, 그리고 LB는? managed addon 설치는? helm 컨트롤러 role은?" → default_node_pools=None 반영(tofu validate로 mode 양쪽과 공존 확인, 2072a20) → LB/managed addon/신원 리서치(managed-identity-overview에서 노드 RG Contributor 자동부여 사실 발견, 우리 설계 문서에 이제껏 없었던 구분) → README+module-catalog.md에 반영(aa176af).

**브랜치 상태**: `feat/azure-aks-cluster-module`, 커밋 4개, **아직 push/PR 안 함**. 다음 세션 첫 액션은 push → PR 오픈.
...
### 2026-08-28 08:44
### 2026-08-28 (세션 5) — PR #38 충돌 해소·merge·문서 stale 정리·aks-cluster-v0.1.0 태그 컷 (azure-aks-cluster 스레드 종결)

세션 시작 시 priority context의 미결 항목("PR #38 merge 여부 다음 세션 확인")을 실측 조회 → 기록과 달리 PR이 CONFLICTING 상태로 바뀐 것 발견(main이 브랜치 분기 후 문서 전용 커밋 2건을 직접 받음). git merge-tree로 실제 충돌 범위 진단(diff --stat만 보면 4개 파일이 벌어진 것처럼 보였지만, 실제 충돌은 .omc/project-memory.json 한 지점뿐이고 나머지는 자동 병합 가능함을 사전 확인) → 사용자에게 AskUserQuestion으로 해소 방식 확인(main을 브랜치에 merge 선택) → 충돌 1건 union으로 해소(a897c64)·push → CI 3게이트 재통과 확인(Monitor로 폴링) → 사용자 승인받아 squash merge(4fc6a48).

merge 직후 README.md·module-catalog.md "Azure 1개" stale 문구 발견(vnet 릴리스 때와 동일 패턴 재발) → Azure 2개로 갱신 + README 표 행 추가 + module-catalog.md aks-cluster 절의 "설계 확정·구현 미착수"·"인터페이스 초안" placeholder를 vnet 절과 동일 패턴(README 링크)으로 정리, main 직접 커밋(5b727f3)·push.

aks-cluster-v0.1.0 태그 컷(사용자 확인 후)·push, git show로 실제 코드(enable_karpenter·private_cluster_enabled) 대조 검증 완료.
