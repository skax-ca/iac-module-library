...
라인 5개(3.5부터 3.9)는 일치한다. 정책 내용이 거의 바뀌지 않아 라인이 유지보수로 넘어가면
상류가 엔진에만 패치를 낸다.

⇒ 티어 안에서 엔진과 정책을 같은 라인에 두고, 정책은 그 라인에서 받을 수 있는 최신 패치에
둔다. 그 라인에 짝이 없는 엔진 패치는 **엔진만 올린다**(보안 백포트가 이 형태로 온다).
라인을 넘는 승격은 지금처럼 4개 값을 짝으로 움직인다.

⛔ 차트 번호 일치를 불변식으로 쓰지 않는다. 번호를 맞춰도 app 버전은 갈릴 수 있다: 엔진
3.3.7은 app v1.13.4인데 같은 라인 정책 최신 3.3.6은 app v1.13.6으로 앞선다. 판정은
`appVersion`으로 한다.
