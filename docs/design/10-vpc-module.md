...
**릴리스 스코프**: 이 문서의 §1이 현행 **`vpc-v0.3.0`** 의 계약이다.

> ⚠️ **2026-08-05 D-VERSION 재매핑** — `vpc-v1.0.0`·`v1.1.0`·`v1.2.0`은 **같은 커밋의**
> `v0.1.0`·`v0.2.0`·`v0.3.0`으로 바뀌었고 구 태그는 삭제됐다
> ([`architecture/05`](../architecture/05-versioning-policy.md)).
> **아래 §3의 릴리스 기록과 §5 열린 항목의 번호는 그때의 사실 기록이라 그대로 둔다** —
> 어느 릴리스에서 무엇이 판정됐는지를 추적할 수 있어야 하기 때문이다. 매핑은 위 세 쌍이 전부다.
>
> **`0.y.z` 구간이므로 계약 변경도 마이너**다(05 §1). 메이저는 `1.0.0` 이후에 의미를 갖는다 —
> 그 전환의 판정 기준은 05 §2이며, `vpc`가 첫 후보다(05 §5-3).
...
## 3. 릴리스 기록 — `vpc-v1.0.0` (2026-07-30) → 현 **`vpc-v0.1.0`**
