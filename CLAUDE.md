...
| 커밋 전 확인 | `verify.yml`이 PR·main push에서 YAML 파싱·root App `include` 판정·로컬 차트 `helm lint`/`template`·`kyverno test`를 돈다(오프라인). `include` 판정은 흉내가 아니라 ArgoCD와 같은 `gobwas/glob`을 같은 방식으로 부른다(`scripts/include-check`). 셸 glob과 달리 `*`도 `/`를 넘는다. ArgoCD의 렌더(Application 조립·파라미터 주입·goTemplate 평가)는 CI가 흉내 내지 않는다. 클러스터가 살아 있으면 `argocd app manifests <app> --core`, 철거 상태면 `helm template --repo <url> <chart> --version <v> -f <values>`로 대체한다. ⚠️ `--core`는 kubeconfig 컨텍스트의 네임스페이스가 `argocd`여야 한다 — 아니면 `configmap "argocd-cm" not found`로 죽는데, 이는 스킬·플래그 문제처럼 보여 오진하기 쉽다. 임시 KUBECONFIG 사본에 `kubectl config set-context --current --namespace=argocd`를 걸어 영구 변경 없이 돌린다. ⚠️ ApplicationSet의 `parameters`는 values 파일에 없으므로 `--set`으로 함께 넘긴다 — 빠뜨리면 렌더가 0건에 `ComparisonError`가 되는데, 이는 차트를 받아오지 못했을 때와 같은 모양이라 값 문제인지 네트워크 문제인지 구분되지 않는다 |
