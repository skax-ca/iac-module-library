...
locals {
  ...
  # ── 노드 풀 이름: 하이픈 금지·12자 한도. 부모(클러스터) 이름이 이미
...
resource "azurerm_kubernetes_cluster" "this" {
  ...
  default_node_pool {
    ...
    # perpetual diff가 된다. azurerm_kubernetes_cluster의
  ...
  identity {
    ...
    # 미리 만든 리소스 ID다. "Currently only one User Assigned Identity is supported"
  ...
  network_profile {
    ...
    # 모드는 provider 기본값(azure)에 맡긴다(cni_mode 변수 설명 참조).
    network_data_plane = local.is_overlay ? "cilium" : null
    outbound_type      = "userAssignedNATGateway"
    # ⛔ network_data_plane이 cilium이면 network_policy도 cilium이어야 한다. provider 공식
    # 문서가 "When network_data_plane is set to cilium, the network_policy field must be
    # set to cilium"이라고 명시하고, ARM도 이를 어기면 "Cilium dataplane requires network
    # policy cilium."으로 거부한다(hashicorp/terraform-provider-azurerm#23339). 짝을 맞추지
    # 않으면 cni_mode = "overlay" 경로가 첫 apply부터 실패한다. 그 외 모드(pod_subnet·
    # node_subnet)는 Azure 자체 정책 엔진("azure")을 쓴다. cilium 데이터플레인이 아니라
    # 이 제약이 적용되지 않는다.
  ...
  # 출력하기 위해서다. 이 모듈은 원시 재료만 내보내고 조합은 소비 루트에 맡긴다.
  oidc_issuer_enabled       = true
  workload_identity_enabled = var.workload_identity_enabled

  # kubelet_identity 블록을 쓰지 않는다. AKS가 노드 리소스 그룹에 자동 생성한다(이 모듈
  # 스코프 밖). 그 auto-created 신원의 object_id는 outputs.tf가 그대로 노출한다.

  # provider 문서는 이 블록을 (Optional)이라 적지만 azurerm 5.3.0의 실제 스키마는 다르다.
  # 블록 자체가 Required이고, 그 안에서 mode 또는 default_node_pools 중 하나가 Required다.
  #
  ...
  #
  # 네트워킹 조합 제약(cni_mode = "pod_subnet"과 함께 쓸 수 없음)은 variables.tf의
  ...
  # 리소스" 충돌을 막으려면) 이 자동 생성을 꺼야 한다. mode가 Auto든 Manual이든 이 고정값과
  # 공존한다.
...
resource "azurerm_kubernetes_cluster_node_pool" "this" {
  ...
  # 전 노드 풀이 pod_subnet_id 하나를 공유한다(풀별 인자이지만 클러스터 단위 단일
  # 입력으로 계약한다). 풀별 오버라이드는 지원하지 않는다. cni_mode가 pod_subnet이 아니면 null
