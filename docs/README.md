...
| **23** | [23-argocd-self-managed.md](design/23-argocd-self-managed.md) | ✅ (2026-08-07 신규 — **D-ARGOCD-SM-BOOTSTRAP**(workbench seed + 자기 관리) · **-REACH**(port-forward) · **-AUTH** · **-HA**. ⚠️ `.tf` 산출물 없음 — 설계상 그렇다. **§5에 `helm` CLI 핀 `v3.21.3` 추가**(2026-08-07 seed 실행 후 — 표에 빠져 있어 실행자가 그 자리에서 골라야 했다. ⚠️ 강제 장치는 여전히 없다 — `helm_version`은 nullable 핀이라 **소비자가 지정해야** 한다. 2026-08-10에 `40`의 변수 설명과 예제를 이 사실에 맞췄고, 소비 repo가 실제로 켜는 것은 별건이다). **§2.3-1 신설**(2026-08-11 — seed 마무리 **실행·완료**: 비밀번호 교체 + `argocd-initial-admin-secret` 삭제. 🔴 **`--core`로는 `update-password`가 실패한다**(코드 근거) → `--port-forward` 로 정정. 웹 UI 2홉 실측. `scripts/argocd-seed.sh`도 함께 보강) |
| **24** | `design/24-argocd-managed-capability.md` | ⏳ **미작성** — 관리형 Capability 경로. 계약면은 [21 §1.7](design/21-gitops-bootstrap-seam.md) |
| 30 | [30-gitops-repo.md](design/30-gitops-repo.md) | 🔶 **부분 개정 2차 + §2.10 신규 집필** (2026-08-11) — 헤더 상태 상자가 절 단위 판정을 소유한다. ✅ **인용 가능**: **§2.10**(승계본이 아니라 이 repo에서 새로 쓴 절 — addon 증분 ②③④ 설계·apply 판정 + ⭐ **§2.10.6 D-SSDIFF**: `OutOfSync` 고착은 **`ServerSideDiff=true` 앱 3개 애노테이션**으로 푼다. `ignoreDifferences`·`managedFieldsManagers`·전역 스위치는 근거를 매겨 기각. 🔴 **§2.10.5의 원인 귀인 2건을 실측으로 정정** — `kube-apiserver`/`kyverno` 는 `status` 서브리소스만 소유했고 진짜 원인은 **apiserver 의 CRD 스키마 defaulting**, mutation webhook 은 무관 ⇒ `IncludeMutationWebhook` 미도입. **둘을 함께 읽는다**) · **§0·§0.1**(경로 무관 — [21 §1.7](design/21-gitops-bootstrap-seam.md)이 판정) · **§1.1**(D-REPO-CODECONNECTIONS 재판정) · **§4.1**(seed 경로별 분기) · **§4.2**(⭐ **D-ROOTAPP-SKIP** — root App 훑기 제외는 `exclude`가 아니라 `+argocd:skip-file-rendering` 마커. 증분 ① 머지가 root App을 **두 번 깨뜨린** 기록: `exclude` 확장이 만든 **자기소멸 데드락** + 마커를 설명하는 주석이 마커로 작동. ⭐ `prune: false`가 root App의 자기 삭제를 막았다) · **§5**(addon 경계 — **인용 제한 해제**, D-ADDON-BOUNDARY가 경로 무관이라). 🔶 **판정 절과 함께만 인용**: **§2.1~§2.5 + [§2.9]**(addon 팬아웃 승계/갈림 · egress·버전 핀 **비승계** → ✅ **실계정 판정 완료**: canary 3호스트 통과 · 핀 `ALBC 3.5.0`·`Karpenter 1.14.0` · ⭐ **판독법 정정** — egress 1차 신호는 rendered가 아니라 `revision` 해석 여부다 · ⭐ **D-ADDON-NS**: 계층 2 addon은 **전용 ns 신설**, 예외는 **ALBC·Karpenter → `kube-system`** 둘뿐(근거 3겹 — Karpenter **APF FlowSchema**·공식 문서·Pod Identity association. ⛔ `system-cluster-critical`의 ns 제약 논거는 **실측 반증** — 되살리지 말 것)) · **§3 + [§3.1]**(AppProject 경로별 값 + ⭐ **선결 과제 2건 → 1건**: self-managed는 chart 기본이 cluster-admin이라 D-ARGOCD-CLUSTER-READ/WRITE 벽이 없다. 남은 1건은 증분과 같은 PR에서 여는 것이라 착수를 막지 않는다. ⛔ 대신 `clusterResourceWhitelist`가 **유일한 실질 가드레일**이 된다). ⚠️ **`§2.6`·`§2.6a`·`§2.7`·`§2.8`은 전부 [20](design/20-eks-module.md)의 절**이다(§2.9 번호 상자). ⇒ 인용 시 **절 번호 필수**. 📌 §1에 `bootstrap/argocd-seed.sh` vendoring **포인터 상자** — 결정 본체는 [40 §2.5](design/40-workbench.md) |
| **40** | [40-workbench.md](design/40-workbench.md) | ✅ (2026-08-07 갱신 — **D-WORKBENCH-REPO**(§2.5, private 저장소 = GitHub App 토큰 + seed 스크립트 vendoring, **부트스트랩 시점 한정**) 신설 · §4.1에 `git`은 변수 없이 항상 설치. ✅ **`.tf` 반영 완료**(2026-08-10, `workbench-v0.2.0` — §2.5 귀결 상자의 *"nullable 패턴으로 열어야 한다"* 는 §4.1이 기각했고 구현은 §4.1을 따랐다. **T-9** 신설). ✅ **열린 항목 7 해소**(2026-08-10, `workbench-v0.3.0` — `argocd` CLI nullable 핀 `v3.5.0`. 결정 본문을 §4.1·§4.3으로 승격하고 항목을 닫았다. **T-10** 양성·음성). 🔴 **D-WORKBENCH-SIZE 신설**(2026-08-10, `workbench-v0.4.0` — 기본 `instance_type` `t4g.nano` → **`t4g.small`**. v0.3.0 첫 apply 에서 **부팅 중 `dnf` 가 OOM-kill** 돼 `git` 이 설치되지 않았다. §7.3-3 에 apply 판정을 남겼다 — **T-9 는 통과했는데 실행이 실패했다**. §9 비용표 월 $5 → $17). 이전: **D-WORKBENCH-MODULE** · **-SCOPE** · **-SEAM** · **-RENAME** + §7.3-1·§7.3-2 apply 판정 + 열린 항목 7 `argocd` CLI 예정) |
