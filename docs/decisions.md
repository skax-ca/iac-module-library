...
전체 결정 이력은 [decisions-aks-cluster.md](decisions-aks-cluster.md)로 분리했다
(400줄 상한 초과, `writing-style.md` 규칙 4). 핵심만: 신원·role assignment를
만들지 않고 입력으로만 받으며, CNI는 `cni_mode`(기본 `overlay`), Entra 통합은
옵트인(`entra_admin_group_object_ids` 또는 `entra_integration_enabled`)이고
로컬 계정은 기본 유지한다.
