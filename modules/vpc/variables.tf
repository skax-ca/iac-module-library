# VPC 모듈 인터페이스 — 설계 docs/design/10-vpc-module.md §1.2
#
# 변수는 설계 §1.2의 관심사 그룹 순서를 따른다(공통 규약 → 주소 공간 → 환경 프로파일 → Flow Logs).
...
variable "naming" {
  description = <<-EOT
    ...
    예: {workload = "acme", env = "dev", region_code = "an2"} → vpc-acme-dev-an2-main
  EOT
  type = object({
    workload    = string
    env         = string
...
variable "vpc_enabled" {
  description = <<-EOT
    kill switch(파괴 방향). false면 이 모듈의 전 리소스를 파기하고 data source 조회까지 건너뛴다(D10).
    참조 대상이 사라진 뒤에도 plan이 통과해야 teardown이 가능하기 때문이다.
...
variable "deletion_protection" {
  description = <<-EOT
    삭제 보호(보호 방향, D12). true면 aws_vpc에 prevent_destroy가 걸려 파괴 계획 자체가 차단된다.
    보호를 켠 상태의 teardown은 2단계다 — deletion_protection = false로 apply한 뒤 vpc_enabled = false.
    이는 결함이 아니라 보호의 정의다.

    기본값이 false인 이유: D10의 teardown 보장이 기본 동작이어야 한다. 보호는 opt-in이다.
  ...
  validation {
    ...
    # 여기서 먼저 잡아 도메인 언어로 안내한다. prevent_destroy는 최종 방어선으로 남는다(D12).
...
variable "secondary_cidr_blocks" {
  description = <<-EOT
    추가 연결할 secondary CIDR 목록(G1). 예: Pod 전용 ["100.64.0.0/16"].
    ⚠️ primary가 10.0.0.0/15 범위 안이면 10.0.0.0/16 대역 secondary는 연결할 수 없다.
    이 제약은 apply 시 API가 검출하므로 plan 기반 test로는 잡히지 않는다(설계 §1.2).
  EOT
  type        = list(string)
  default     = []
}
...
variable "az_count" {
  description = <<-EOT
    최대 AZ 슬라이스 수. 그룹별 실제 AZ 수는 subnet_groups[*].cidrs의 길이가 결정한다(D6).
...
variable "az_selection" {
  description = <<-EOT
    AZ suffix 우선순위 목록(D7). 각 그룹은 이 목록의 앞에서부터 length(cidrs)개 AZ에 배치된다.
...
variable "subnet_groups" {
  description = <<-EOT
    서브넷 그룹 정의. 키가 곧 Name 태그의 purpose 토큰이 된다(설계 §1.3).
      type       - "public" | "private" | "isolated". 라우팅과 RT 구성을 결정한다(D1/D3)
      cidrs      - AZ 순서대로 명시한 CIDR. 리스트 길이가 곧 그 그룹의 AZ 수다(D6)
      eks_role   - "elb" | "internal-elb" | null. EKS 서브넷 디스커버리 태그를 붙인다(D4)
      extra_tags - 그룹 단위 추가 태그
    그룹 키는 <용도 축약>-<uniq|dup> 형식을 권고하나 모듈이 강제하지는 않는다(설계 §1.3).
    2 <= length(cidrs) <= az_count 제약은 main.tf의 precondition이 검증한다(D6).
  EOT
  type = map(object({
    type       = string
    cidrs      = list(string)
...
variable "eks_cluster_name" {
  description = <<-EOT
    EKS 서브넷 디스커버리 태그(kubernetes.io/cluster/<name>)에 쓸 클러스터 이름(D4).
...
variable "flow_logs_enabled" {
  description = "VPC Flow Logs 생성 여부. 대상은 CloudWatch Logs로 고정한다(D11)."
  type        = bool
  default     = true
}
...
variable "flow_logs_retention_days" {
  ...
  validation {
    # aws provider 6.56.0 문서 실측값 — 임의로 늘리거나 줄이지 않는다.
    condition = contains(
...
variable "flow_logs_kms_key_id" {
  description = <<-EOT
    ...
    KMS 키는 계정 전역 공유·보안 거버넌스 대상이라 모듈이 생성하지 않고 주입받는다(03 §4).
